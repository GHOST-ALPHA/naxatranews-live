            ...
            {media.length === 0 ? (
                ...
                {media.map((item: any) => (
                  ...
                  >
                    <div className="aspect-square relative bg-background w-full">
                      ...
                      {/* Hover overlay - Hidden on mobile, actions in Detail Dialog instead */}
                      <div className="hidden sm:flex absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity items-center justify-center gap-2">
                        <Button variant="secondary" size="icon" className="h-8 w-8 rounded-full" onClick={(e) => { e.stopPropagation(); setSelectedMedia(item); setDetailOpen(true) }}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="destructive" size="icon" className="h-8 w-8 rounded-full" onClick={(e) => { e.stopPropagation(); setMediaToDelete(item.id); setDeleteDialogOpen(true) }}>
              ...
              <Card>
                <div className="divide-y">
                  {media.map((item: any) => (
                    <div key={item.id} className="flex items-center gap-3 p-3 hover:bg-muted/50 transition-colors group">
                      <div className="relative w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden bg-background shrink-0">
                        {item.resourceType === "image" ? (
