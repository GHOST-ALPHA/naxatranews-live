...
export function CommentsSection({ newsId, count: initialCount }: CommentsSectionProps) {
  ...
  function formatDate(date: Date | string) {
    ...
    const diff = nowIST.getTime() - dIST.getTime()
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex items-center gap-5 mt-3 pt-2">
              <button
                ...
                className="flex items-center gap-1.5 text-xs md:text-sm font-semibold text-muted-foreground hover:text-[#FF6B35] transition-colors"
              >
  ...
  return (
    <div className="mb-12 scroll-mt-24" id="comments">
      <Button
        ...
        className="w-full py-8 text-base md:text-lg border-border hover:border-orange-300 text-foreground hover:text-[#FF6B35] font-bold uppercase tracking-wider bg-card hover:bg-orange-50/50 dark:hover:bg-orange-950/20 flex items-center justify-between px-6 md:px-10 group transition-all shadow-sm"
      >
        <span className="flex items-center gap-3">
          <span className="flex items-center justify-center w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-900/40 text-[#FF6B35] dark:text-orange-400 group-hover:scale-110 transition-transform">
        ...
        <div className="mt-8 space-y-8 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <form onSubmit={handleSubmit} className="flex gap-3 md:gap-5 bg-muted/30 p-4 md:p-6 rounded-xl border border-border/50">
            <Avatar className="h-10 w-10 md:h-12 md:w-12 shrink-0 border-2 border-background shadow-sm hidden sm:block">
              <AvatarFallback className="bg-orange-100 text-[#FF6B35] font-bold">
                {formData.authorName[0]?.toUpperCase() || "?"}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 space-y-3">
              {replyTo && (
                <div className="flex items-center justify-between bg-orange-50 dark:bg-orange-950/20 p-2 rounded text-sm">
                  <span className="text-[#FF6B35] dark:text-orange-400">Replying to comment</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  ...
                  className="w-full p-3 border border-input rounded-lg focus:ring-2 focus:ring-[#FF6B35]/20 focus:border-[#FF6B35] bg-background text-sm"
                  required
                />
                <input
                  type="email"
              ...
              <textarea
                ...
                className="w-full p-4 border border-input rounded-lg focus:ring-2 focus:ring-[#FF6B35]/20 focus:border-[#FF6B35] min-h-[120px] bg-background text-base resize-y transition-all placeholder:text-muted-foreground/70"
                required
              />
              <div className="flex items-center justify-between">
                ...
                <Button
                  type="submit"
                  disabled={submitting}
                  className="bg-[#FF6B35] hover:bg-[#E55A2B] text-white px-6 shadow-sm hover:shadow w-full sm:w-auto"
                >
