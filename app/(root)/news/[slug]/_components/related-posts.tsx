...
export function RelatedPosts({ posts }: RelatedPostsProps) {
  ...
  return (
    <div className="mb-12 border-t border-border pt-8">
      <h3 className="text-xl font-bold mb-4 uppercase border-b border-[#FF6B35] inline-block pb-1 text-[#FF6B35]">
        Related Posts
      </h3>
        ...
        {posts.map((post) => (
          <Link href={`/news/${post.slug}`} key={post.id} className="group block h-full">
            <div className="flex flex-col h-full">
              ...
              <h4 className="font-bold text-md mb-2 group-hover:text-red-600 transition-colors line-clamp-3 leading-snug">
                {post.title}
              </h4>
