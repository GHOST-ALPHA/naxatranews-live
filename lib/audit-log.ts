...
export async function createAuditLog(
  ...
  try {
    ...
    // Verify user exists before creating audit log to prevent foreign key constraint violations
    ...
    if (!userExists) {
      console.warn(`Cannot create audit log: User ${finalUserId} does not exist`);
      return;
    }
    ...
    if (error instanceof Error) {
      ...
      const isForeignKeyError = error.message.includes("Foreign key constraint") || 
                                error.message.includes("audit_logs_userId_fkey");
      
      if (isForeignKeyError) {
        console.warn(`Cannot create audit log: User does not exist (foreign key constraint)`);
        return;
      }
