...
export const getNewsViewsByDeviceType = cache(async (days: number = 30) => {
  try {
    ...
    // Get visits with device info - include all visits to news pages
    ...
    visits.forEach((visit: any) => {
      ...
      // Use device field if available, otherwise try to infer from userAgent
      let device = visit.device;
      
      if (!device && visit.userAgent) {
      ...
      device = device || "desktop"; // default to desktop if still unknown
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      if (dailyDeviceCounts[dateStr]) {
        Object.entries(dailyDeviceCounts[dateStr]).forEach(([device, count]) => {
          let normalizedDevice = "desktop"; // default
