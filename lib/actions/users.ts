...
export async function getUserStats() {
  try {
    ...
    const hasAccess = await hasPermission(currentUser.userId, "user.read");
    if (!hasAccess) {
      return { success: false, error: "You don't have permission to view stats" };
    }

    const now = new Date();
    const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate());

    const [totalUsers, activeUsers, newUsersToday, usersByRole] = await Promise.all([
      prisma.user.count(),
    ...
    const roles = await prisma.role.findMany({
      where: {
        id: { in: usersByRole.map((r) => r.roleId) },
      },
    });
