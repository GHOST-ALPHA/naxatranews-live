...
export interface NewsResponse {
  ...
  coverVideo: string | null; // YouTube, Vimeo, or direct video URL
  isBreaking: boolean;
  isFeatured: boolean;
  viewCount: number;
  likes: number;
...
async function getCategoryIds(filter: CategoryFilter): Promise<string[]> {
  ...
  if (filter.parentSlug) {
    // Get parent category - optimized: query by unique slug first, then filter
    ...
    // Get specific category - optimized: query by unique slug first, then filter
